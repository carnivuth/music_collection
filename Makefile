...
$(collection_dir)/%.lrc:  $(collection_dir)/$(firstword $(wildcard %.opus %.m4a %.wav %.flac %.mp3))
	curl -X GET 'https://lrclib.net/api/search?track_name=$(shell echo '$*' | sed -n 's/.*[0-9][0-9]-[0-9][0-9]\. \(.*\)/\1/p')&artist_name=$(shell echo '$*' | awk -F'/' '{print $$1}')&album_name=$(shell echo '$*' | awk -F'/' '{print $$2}' | tr ' ' '+' )' | \
